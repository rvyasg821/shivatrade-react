...
const ImportModal = ({ isOpen, toggle, onSuccess }) => {
  ...
  const instructions = (
    <ol className="mb-0 mt-1">
      ...
      <li>{t("Rows are matched by code — a matching code updates that rebate, a new code creates one")}</li>
  ...
  return (
    <SharedImportModal
      ...
      toggle={toggle}
      onSuccess={onSuccess}
      title={t("Import Rebates")}
      importUrl={API_ENDPOINTS.rebates.import}
      sampleUrl={API_ENDPOINTS.rebates.sampleExcel}
      sampleFilename="rebate-import-sample.xlsx"

...
const RebateList = () => {
  ...
  const handleLists = useCallback(
    (sorting = sort, sortCol = sortColumn, page = currentPage, perPage = rowsPerPage, search = searchInput, status = statusFilter) => {
      dispatch(getRebateList({ orderBy: sortCol, orderDirection: sorting, page, perPage, search, status }));
    },
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await instance.get(API_ENDPOINTS.rebates.export, {
        responseType: "blob",
      });
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement("a");
      a.href = url;
      a.download = `rebates-${new Date().toISOString().split("T")[0]}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
    } catch {
      Notification("Error", t("Failed to export rebates"), "warning");
    } finally {
      setExporting(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (store?.actionFlag === "REB_DLT") handleLists();
  ...
  const columns = [
    {
      ...
      selector: (row) => canEdit ? (
        <Link to={`${appsRoot}/rebates/edit/${row?._id || ""}`} className="text-capitalize text-wrap">
          {row?.name || ""}
        </Link>
    ...
    columns.push({
      ...
      cell: (row) => (
        <div className="d-flex column-action align-items-center table-icon">
          {canEdit && (
            <Link className="me-50" id={`reb-edit-${row?._id}`} to={`${appsRoot}/rebates/edit/${row?._id}`}>
              <UncontrolledTooltip placement="top" target={`reb-edit-${row?._id}`}>{t("Edit")}</UncontrolledTooltip>
              <Edit size={20} />
            </Link>
          )}
          {canDelete && (
            <>
              <Trash2 size={20} className="cursor-pointer" id={`reb-del-${row?._id}`} onClick={() => handleDelete(row?._id)} />
              <UncontrolledTooltip placement="top" target={`reb-del-${row?._id}`}>{t("Delete")}</UncontrolledTooltip>
            </>
          )}
        </div>
      ),
  ...
  return (
    <Fragment>
      <div className="main-content rebates">
        <div className="d-flex align-items-center justify-content-between mb-2">
          <h3 className="mb-0">{t("Rebates")}</h3>
        </div>
          ...
          <CardBody>
            <div className="d-flex align-items-center flex-nowrap gap-2">
                ...
                <Row>
                  <Col sm="6" md="6" className="mb-2 mb-md-0">
                    <Input type="text" value={searchInput} className="w-100 select"
                      placeholder={t("Search Rebates")} onChange={(e) => handleSearch(e?.target?.value)} />
                  </Col>
                  <Col sm="6" md="6" className="mb-2 mb-md-0">
                    <Select
              ...
              <div className="d-flex align-items-center justify-content-end gap-1 flex-shrink-0">
                  ...
                  {canAdd && (
                    <Button color="primary" className="text-nowrap" onClick={() => navigate(`${appsRoot}/rebates/add`)}>
            ...
            <Row className="mt-2">
              <Col md="12" className="rebate-tables">
                <DatatablePagination
                  columns={columns}
                  data={store?.rebateItems || []}

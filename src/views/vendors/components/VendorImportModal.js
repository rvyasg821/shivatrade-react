...
const VendorImportModal = ({ isOpen, toggle, onSuccess }) => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const res = await instance.post(
        `${API_ENDPOINTS.vendors.import}?preview=true`,
        formData,
  ...
  const handleDownloadSample = async () => {
    try {
      const res = await instance.get(API_ENDPOINTS.vendors.sampleExcel, {
        responseType: "blob",
      });
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement("a");
      a.href = url;
      a.download = "vendor-import-sample.xlsx";
      a.click();
          ...
          <div>
            <Alert color="info" className="mb-2">
              ...
              <ol className="mb-0 mt-1">
                ...
                <li>
                  {t(
                    "Required columns: company_name, name, email, phone"
                  )}
                </li>
                <li>
                  {t(

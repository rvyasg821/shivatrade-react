...
const PaymentForm = ({
    ...
    useEffect(() => {
        if (companyId) {
            dispatch(getActiveCard({ userId: companyId, gateway: gateway }));
        }
    }, [companyId, gateway, dispatch]);
    ...
    // Label based on gateway
    const cardLabel = gateway === 'stripe' ? t("Stored Stripe Cards") : t("Stored PayPal Cards");
    const noCardMessage = gateway === 'stripe'
        ? t("There is no Stripe card available.")
        : t("There is no PayPal card available.");
    const securityMessage = gateway === 'stripe'
        ? t("Your payment information is encrypted and secure. Powered by Stripe.")
    ...
    return (
        <form className="payment-form">
            {/* Active Card Display */}
            <div className="mb-3">
                <Label className="form-label">{cardLabel}</Label>
                    ...
                    render={({ field }) => (
                        <div>
                            {activeCardLoading ? (
                                ...
                                <p className="text-muted">{noCardMessage}</p>
                            )}
                        </div>
                    )}
                />
            ...
            <div className="security-notice d-flex align-items-center">
                ...
                <small className="text-muted">{securityMessage}</small>
            </div>
